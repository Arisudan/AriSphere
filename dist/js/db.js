...
const CATEGORIES = {
  ...
  reflections: { id: 'reflections', name: 'Reflections', desc: 'Personal stories, experiences, lessons, observations, and human perspectives on technology, education, careers, entrepreneurship, and modern life.' }
};
...
const ARTICLES = [
  ...
  {
    id: 6,
    title: 'Building AriSphere From Zero to Production',
    subtitle: 'A post-mortem on code refactoring, JSDOM lifecycle race conditions, and moving to a Postgres CMS.',
    excerpt: 'Building a modern web application requires balancing architecture decisions. Here is how I structured AriSphere\'s static compiler and database fallback layer.',
    content: `
    ...
    tags: ['Web Development', 'Static Site Generator', 'PostgreSQL', 'Architecture'],
    featured: false,
    trending: true,
    trendingThisWeek: true,
    editorsPick: true,
  ...
  {
    id: 7,
    title: 'What Jayem Automotive Taught Me About Real Engineering',
    subtitle: 'Moving past academic textbooks to understand physical mechanics, hardware limits, and industrial timelines.',
    excerpt: 'My time at Jayem Automotive redefined what engineering means to me. Here are the core lessons from the shop floor and building real hardware.',
    content: `
    ...
    tags: ['Mechanical Engineering', 'Automotive', 'Prototyping', 'Lessons Learned'],
    featured: false,
    trending: true,
    trendingThisWeek: false,
    editorsPick: true,
  ...
  {
    id: 8,
    title: 'The Reality of Pursuing Embedded Systems in India',
    subtitle: 'An honest analysis of hardware ecosystems, career pathways, and the skills needed to bridge the educational gap.',
    excerpt: 'Finding hardware roles in India is challenging. I analyze the market realities and how students can navigate them.',
    content: `
    ...
    tags: ['Embedded Systems', 'India Career', 'Firmware', 'PCB Design'],
    views: 1540
  }
];
...
async function fetchFromSupabase(endpoint) {
  ...
  if (typeof window !== 'undefined' && window.AriSphereDB && window.AriSphereDB.supabase) {
    try {
      const { data } = await window.AriSphereDB.supabase.auth.getSession();
      if (data && data.session) {
        token = data.session.access_token;
      }
    } catch (e) {
      console.warn("Error getting Supabase session in fetch:", e);
    }
  }
...
// Adapter to map database table snake_case columns to application camelCase properties
function mapDatabaseArticle(row) {
  ...
  const mapped = {
    ...
    trendingThisWeek: !!row.trending_this_week,
  ...
  else if (mapped.id === 2) mapped.sourceAttribution = 'Bloomberg Supply Chain Reports';
  else if (mapped.id === 3) mapped.sourceAttribution = 'IBM Quantum Labs & IEEE Spectrum';
  else if (mapped.id === 4) mapped.sourceAttribution = 'Pew Research Center Analytics';
  else if (mapped.id === 5) mapped.sourceAttribution = 'TSMC Technical Disclosures & Taiwan Tech';
  else mapped.sourceAttribution = 'AriSphere Editorial Desk';
...
async function getArticleById(id, includeDrafts = false) {
  if (isSupabaseConfigured()) {
    try {
      ...
      const data = await fetchFromSupabase(`articles?id=eq.${id}${statusFilter}&select=*`);
      if (data && data.length > 0) {
        return mapDatabaseArticle(data[0]);
      }
    } catch (err) {
  ...
  if (found && (includeDrafts || (found.status || 'published') === 'published')) {
    return found;
  }
  return null;
}

async function getArticlesByCategory(category, includeDrafts = false) {
  if (isSupabaseConfigured()) {
    try {
      const statusFilter = includeDrafts ? '' : '&status=eq.published';
      if (category === 'trending') {
        const data = await fetchFromSupabase(`articles?trending=eq.true${statusFilter}&select=*`);
        return data.map(mapDatabaseArticle);
      }
      const data = await fetchFromSupabase(`articles?category=eq.${category}${statusFilter}&select=*`);
      return data.map(mapDatabaseArticle);
    } catch (err) {
      console.warn('Supabase fetch failed, using local DB fallback.', err);
    }
  ...
  if (category === 'trending') {
    return ARTICLES.filter(a => a.trending && (includeDrafts || (a.status || 'published') === 'published'));
  }
  return ARTICLES.filter(a => a.category === category && (includeDrafts || (a.status || 'published') === 'published'));
}

async function getArticlesByAuthor(authorUsername, includeDrafts = false) {
  if (isSupabaseConfigured()) {
    try {
      const statusFilter = includeDrafts ? '' : '&status=eq.published';
      const data = await fetchFromSupabase(`articles?author=eq.${authorUsername}${statusFilter}&select=*`);
      return data.map(mapDatabaseArticle);
    } catch (err) {
      console.warn('Supabase fetch failed, using local DB fallback.', err);
    }
  }
  return ARTICLES.filter(a => a.author === authorUsername && (includeDrafts || (a.status || 'published') === 'published'));
}

async function searchArticles(query, includeDrafts = false) {
  if (!query) return [];
  const q = query.toLowerCase().trim();
  if (isSupabaseConfigured()) {
    try {
      const statusFilter = includeDrafts ? '' : '&status=eq.published';
      const data = await fetchFromSupabase(`articles?or=(title.ilike.*${q}*,subtitle.ilike.*${q}*,excerpt.ilike.*${q}*)${statusFilter}&select=*`);
      return data.map(mapDatabaseArticle);
    } catch (err) {
      console.warn('Supabase fetch failed, using local DB fallback.', err);
    }
  ...
  return ARTICLES.filter(a =>
    (includeDrafts || (a.status || 'published') === 'published') && (
...
async function getRelatedArticles(currentArticleId, limit = 3) {
  ...
  if (isSupabaseConfigured()) {
    try {
      const data = await fetchFromSupabase(`articles?id=neq.${current.id}&status=eq.published&select=*`);
      if (data && data.length > 0) candidates = data.map(mapDatabaseArticle);
    } catch (err) {
      console.warn('Supabase fetch failed in getRelatedArticles, using local DB fallback.', err);
      candidates = ARTICLES.filter(a => String(a.id) !== String(current.id) && (a.status || 'published') === 'published');
    }
  } else {
    candidates = ARTICLES.filter(a => String(a.id) !== String(current.id) && (a.status || 'published') === 'published');
  }
  ...
  const scored = candidates
    .map(a => {
      ...
      const candidateTitleWords = a.title.toLowerCase().split(/\W+/).filter(w => w.length > 2 && !stopWords.has(w));
      const matchingWords = candidateTitleWords.filter(w => currentTitleWords.includes(w));
...
async function getTrendingArticles(limit = 5, includeDrafts = false) {
  if (isSupabaseConfigured()) {
    try {
      const statusFilter = includeDrafts ? '' : '&status=eq.published';
      const data = await fetchFromSupabase(`articles?trending=eq.true${statusFilter}&limit=${limit}&select=*`);
      return data.map(mapDatabaseArticle);
    } catch (err) {
      console.warn('Supabase fetch failed, using local DB fallback.', err);
    }
  }
  return ARTICLES.filter(a => a.trending && (includeDrafts || (a.status || 'published') === 'published')).slice(0, limit);
}

async function getTrendingThisWeek(limit = 5, includeDrafts = false) {
  if (isSupabaseConfigured()) {
    try {
      const statusFilter = includeDrafts ? '' : '&status=eq.published';
      const data = await fetchFromSupabase(`articles?trending_this_week=eq.true${statusFilter}&limit=${limit}&select=*`);
      return data.map(mapDatabaseArticle);
    } catch (err) {
      console.warn('Supabase fetch failed, using local DB fallback.', err);
    }
  }
  return ARTICLES.filter(a => a.trendingThisWeek && (includeDrafts || (a.status || 'published') === 'published')).slice(0, limit);
}

async function getEditorsPicks(limit = 4, includeDrafts = false) {
  if (isSupabaseConfigured()) {
    try {
      const statusFilter = includeDrafts ? '' : '&status=eq.published';
      const data = await fetchFromSupabase(`articles?editors_pick=eq.true${statusFilter}&limit=${limit}&select=*`);
      return data.map(mapDatabaseArticle);
    } catch (err) {
      console.warn('Supabase fetch failed, using local DB fallback.', err);
    }
  }
  return ARTICLES.filter(a => a.editorsPick && (includeDrafts || (a.status || 'published') === 'published')).slice(0, limit);
}

async function getMostReadArticles(limit = 5, includeDrafts = false) {
  if (isSupabaseConfigured()) {
    try {
      const statusFilter = includeDrafts ? '' : '&status=eq.published';
      const data = await fetchFromSupabase(`articles?order=views.desc${statusFilter}&limit=${limit}&select=*`);
      return data.map(mapDatabaseArticle);
    } catch (err) {
      console.warn('Supabase fetch failed, using local DB fallback.', err);
    }
  }
  return [...ARTICLES].filter(a => includeDrafts || (a.status || 'published') === 'published').sort((a, b) => b.views - a.views).slice(0, limit);
}

async function getLatestArticles(limit = 8, includeDrafts = false) {
  if (isSupabaseConfigured()) {
    try {
      const statusFilter = includeDrafts ? '' : '&status=eq.published';
      const data = await fetchFromSupabase(`articles?order=id.desc${statusFilter}&limit=${limit}&select=*`);
      return data.map(mapDatabaseArticle);
    } catch (err) {
      console.warn('Supabase fetch failed, using local DB fallback.', err);
    }
  }
  return [...ARTICLES].filter(a => includeDrafts || (a.status || 'published') === 'published').sort((a, b) => b.id - a.id).slice(0, limit);
}

async function getFeaturedArticle(includeDrafts = false) {
  if (isSupabaseConfigured()) {
    try {
      const statusFilter = includeDrafts ? '' : '&status=eq.published';
      const data = await fetchFromSupabase(`articles?featured=eq.true${statusFilter}&limit=1&select=*`);
      if (data && data.length > 0) return mapDatabaseArticle(data[0]);
    } catch (err) {
      console.warn('Supabase fetch failed, using local DB fallback.', err);
    }
  }
  return ARTICLES.find(a => a.featured && (includeDrafts || (a.status || 'published') === 'published')) || ARTICLES.find(a => includeDrafts || (a.status || 'published') === 'published');
}
...
async function createArticleAdmin(articleData) {
...
async function updateArticleAdmin(id, articleData) {
  if (isSupabaseConfigured() && supabaseClient) {
    const dbData = {
      title: articleData.title,
      subtitle: articleData.subtitle,
    ...
    throw new Error('Article not found locally.');
  }
}
